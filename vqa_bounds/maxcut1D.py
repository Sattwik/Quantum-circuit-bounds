...
class MaxCut1D(meta_system.System):
    ...
    def noise_layer(self, var_tensors: Dict):
        ...
        for site_tuple, local_var_tensor in var_tensors.items():
            ...
            local_res_tensor = (1 - 3 * self.p/4) * local_var_tensor + (self.p/4) * \
                (jnp.matmul(self.X_right, jnp.matmul(local_var_tensor, self.X_right)) + \
                 jnp.matmul(self.Y_right, jnp.matmul(local_var_tensor, self.Y_right)) + \
                 jnp.matmul(self.Z_right, jnp.matmul(local_var_tensor, self.Z_right)))
