...
class MaxCut2DPlanar(meta_system.System):
    ...
    def Um(self, beta):
        ...
        for site in self.graph:
            ...
            Um *= qutip.tensor([self.I_qutip] * site_num + [Ux] + \
    ...
    def circ_grad(self, circ_params: np.array):
        ...
        gradient_gamma = np.zeros(self.d)
        gradient_beta = np.zeros(self.d)
        ...
        for step_num in range(self.d):
            ...
            objective_plus = self.circ_obj(gamma_plus, beta)
            objective_minus = self.circ_obj(gamma_minus, beta)
            ...
            objective_plus = self.circ_obj(gamma, beta_plus)
            objective_minus = self.circ_obj(gamma, beta_minus)
