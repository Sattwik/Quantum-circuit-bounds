...
class MaxCut1DLocalPurityLambda(meta_system.System):
    ...
    def cost(self):
        ...
        # the entropy term
        cost = jnp.dot(self.Lambdas.at[self.d_purity:].get(), self.entropy_bounds.at[self.d_purity:].get())
        cost += -jnp.dot(self.Lambdas.at[:self.d_purity].get(), self.purity_bounds.at[:self.d_purity].get())
        ...
        for i in range(self.d_purity):
            ...
            cost += -jnp.trace(Hi_squared)/(4 * self.Lambdas[i])
