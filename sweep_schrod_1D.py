...
def sweep_schrod(N_list, p_list, theta_list, d_list, D_list):
    max_threads = 9
...
def submit_simulation(N, d, seed, p, D, theta, result_save_path):
    ...
    params = ['python', 'single_schrod_1D.py',
              '--N', N,
              '--d', d,
              '--seed', seed,
              '--p', p,
    ...
    fname = "schrod1D-N-" + str(N) + "-d-" + str(d) + "-seed-" + \
...
sweep_schrod(N_list, p_list, theta_list, d_list, D_list)
