...
def submit_simulation(N, d, seed, p, k_dual, result_save_path):
    ...
    params = ['python', 'single_fermion_2D_vs_depth.py',
              '--N', N,
              '--d', d,
              '--seed', seed,
              '--p', p,
    ...
    fname = "fermion2D-block-purity-N-" + str(N) + "-d-" + str(d) + "-seed-" + str(seed) + "-p-" + str(p) + "-kdual-" + \
                 str(k_dual) + ".txt"
...
N_list = [7]
p_list = [0.05]
d_list = np.arange(8, 50, 8)
k_dual_list = [1, 3, 4, 5, 6, 7, 9, 12]
