...
# approx ratios vs N = 32-72 for p = 1%-10%, block arrangement, depth = N/2
data_path = "./../vqa_data/1202/20221202-155635"
...
N_list = [32, 36, 40, 44, 48, 52, 56, 60, 64, 68, 72, 72]
    ...
    for i_seed, seed in enumerate(N + np.array(range(num_seeds))):
        for i_p, p in enumerate(p_list):
            ...
            for i_k, k_dual in enumerate([1]):
                fname = "fermion1D-proj-block-N-" + str(N) + "-seed-" + str(seed) + \
                        "-p-" + str(p) + "-kdual-" + \
                        str(k_dual) + ".pkl"
                with open(os.path.join(data_path, fname), 'rb') as result_file:
                    print("Reading file: " + os.path.join(data_path, fname))

                    clean_sol, noisy_sol, noisy_bound, noisy_bound_proj, noisy_bound_nc, \
                    lambda_lower_bounds, \
                    ...
                    noisy_bound_proj_list[i_N, i_seed, i_p, i_k] = noisy_bound_proj
...
approx_ratio_noisy_bound_proj = noisy_bound_proj_list/clean_sol_list
...
#
#     for i_k, k_dual in enumerate([1]):
#         mean_approx_noisy_bound_proj = np.mean(approx_ratio_noisy_bound_proj[:, :, i_p, i_k], axis = 1)
#         std_approx_noisy_bound_proj = np.std(approx_ratio_noisy_bound_proj[:, :, i_p, i_k], axis = 1)
#
#         ax.plot(N_list, mean_approx_noisy_bound_proj, marker = '.', color = CB_color_cycle[i_k + 1], label = r'Proj. dual (k = ' + str(k_dual) + ')')
#         ax.fill_between(N_list,
#                         (mean_approx_noisy_bound_proj - std_approx_noisy_bound_proj),
#                         (mean_approx_noisy_bound_proj + std_approx_noisy_bound_proj),
