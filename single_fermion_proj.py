...
dual_vars_proj_init = jnp.zeros((d,))
dual_obj_over_opti_proj, dual_opt_result_proj = \
    gaussian.optimize(dual_vars_proj_init, dual_params,
...
print("noisy bound proj = ", noisy_bound_proj)
...
data_list = [clean_sol, noisy_sol, noisy_bound, noisy_bound_proj, noisy_bound_nc,
             lambda_lower_bounds,
...
data_file_name = "fermion1D-proj-block-N-" + str(N) + "-seed-" + cliargs.seed + "-p-" + str(p) + "-kdual-" + \
                 str(k_dual) + ".pkl"
