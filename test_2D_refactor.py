...
from vqa_bounds import maxcut2Dplanar, graphs, circuit_utils, dual_utils
...
# m = 2
# n = 3
# lattice = graphs.define_lattice((m, n))
#
# graph = graphs.create_random_connectivity(lattice)
#
# d = 2
#
# sys_obj = maxcut2Dplanar.MaxCut2DPlanar(graph, lattice, d, p = 0)
#
# gamma = np.zeros(d)
# beta = np.zeros(d)
#
...
# sys_obj.update_opt_circ_params(np.concatenate((gamma, beta)))
#
# state = sys_obj.circuit_layer(layer_num = 0, var_tensor = sys_obj.rho_init_tensor, mode = "primal")
# state = sys_obj.circuit_layer(layer_num = 1, var_tensor = state, mode = "primal")
#
# state_dm = sys_obj.tensor_2_mat(state.tensor)
#
# psi_after_step = sys_obj.init_state()
# psi_after_step = sys_obj.Up(gamma[0]) * psi_after_step
# psi_after_step = sys_obj.Um(beta[0]) * psi_after_step
#
# psi_after_step = sys_obj.Up(gamma[1]) * psi_after_step
# psi_after_step = sys_obj.Um(beta[1]) * psi_after_step
#
...
primal_actual = np.min(sys_obj.H.full())
