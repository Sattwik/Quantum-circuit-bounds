...
data_path = "./../vqa_data/1223/20211223-191154"
...
noisy_bound_list = []
noisy_bound_nc_list = []
noisy_sol_global_list = []
noisy_bound_global_list = []
graphs_list = []
...
approx_ratio_noisy_bound_nc = noisy_bound_nc/actual_sol
...
approx_ratio_noisy_bound_nc = np.reshape(approx_ratio_noisy_bound_nc, (num_noise_probs, num_random_graphs * num_init_states))
...
mean_approx_noisy_bound_nc = np.mean(approx_ratio_noisy_bound_nc, axis = 1)
std_approx_noisy_bound_nc = np.std(approx_ratio_noisy_bound_nc, axis = 1)
...
ax.plot(p_noise_list, mean_approx_noisy_bound_nc, marker = '.', color = CB_color_cycle[1], label = r'Bound (no channel)')
ax.fill_between(p_noise_list, (mean_approx_noisy_bound_nc - std_approx_noisy_bound_nc), (mean_approx_noisy_bound_nc + std_approx_noisy_bound_nc), color = CB_color_cycle[1], alpha = 0.1)

ax.plot([0, 1], [mean_approx_clean, mean_approx_clean], ls = "-.", lw = 1, color = CB_color_cycle[2], label = r'Clean')
ax.fill_between(p_noise_list, [(mean_approx_clean - std_approx_clean)] * len(p_noise_list), [(mean_approx_clean + std_approx_clean)] * len(p_noise_list), color = CB_color_cycle[2], alpha = 0.1)
...
duality_gap_nc = noisy_sol - noisy_bound_nc
duality_gap_nc = np.reshape(duality_gap_nc, (num_noise_probs, num_random_graphs * num_init_states))
mean_duality_gap_nc = np.mean(duality_gap_nc, axis = 1)
std_duality_gap_nc = np.std(duality_gap_nc, axis = 1)

duality_gap_global = noisy_sol_global - noisy_bound_global
duality_gap_global = np.reshape(duality_gap_global, (num_noise_probs, num_random_graphs * num_init_states))
mean_duality_gap_global = np.mean(duality_gap_global, axis = 1)
std_duality_gap_global = np.std(duality_gap_global, axis = 1)
